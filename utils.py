...
def plot_surfaces(grad_dict, index, outdir, prefix, normalize=False, cmap='jet'):
    ...
    plotting.plot_surf_stat_map(grad_dict['pial_left'], grad_lh, hemi='left', bg_map=grad_dict['sulc_left'], bg_on_data = True, threshold=np.finfo(np.float32).eps, colorbar=False, view='medial', cmap=cmap,  output_file=op.join(outdir, '{0}-{1}_left_medial.png'.format(prefix, index)))
    plotting.plot_surf_stat_map(grad_dict['pial_left'], grad_lh, hemi='left', bg_map=grad_dict['sulc_left'], bg_on_data = True, threshold=np.finfo(np.float32).eps, colorbar=False, view='lateral', cmap=cmap, output_file=op.join(outdir, '{0}-{1}_left_lateral.png'.format(prefix, index)))

    plotting.plot_surf_stat_map(grad_dict['pial_right'], grad_rh, hemi='right', bg_map=grad_dict['sulc_right'], bg_on_data = True, threshold=np.finfo(np.float32).eps, colorbar=False, view='medial', cmap=cmap, output_file=op.join(outdir, '{0}-{1}_right_medial.png'.format(prefix, index)))
    plotting.plot_surf_stat_map(grad_dict['pial_right'], grad_rh, hemi='right', bg_map=grad_dict['sulc_right'], bg_on_data = True, threshold=np.finfo(np.float32).eps, colorbar=True, view='lateral', cmap=cmap, output_file=op.join(outdir, '{0}-{1}_right_lateral.png'.format(prefix, index)))
...
def combine_plots(list_im, fname_out):
    ...
    imgs_comb = np.hstack( (np.asarray( i.resize((int(i.size[0]*i.size[1]/imgs[3].size[1]), imgs[3].size[1])) ) for i in imgs ) )
