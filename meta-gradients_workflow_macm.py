...
def plot_surfaces(grad_dict, index, outdir):
    ...
    if np.max(grad_lh[grad_lh>0]) > np.abs(np.min(grad_lh[grad_lh<0])):
        ...
        grad_lh[grad_lh>0] = grad_lh[grad_lh>0]/np.max(grad_lh[grad_lh>0])
        grad_lh[grad_lh>0] = grad_lh[grad_lh>0]*np.abs(np.min(grad_lh[grad_lh<0]))
    plotting.plot_surf_stat_map(grad_dict['pial_left'], grad_lh, hemi='left', bg_map=grad_dict['sulc_left'], bg_on_data = True, colorbar=False, view='medial', cmap='jet',  output_file=op.join(outdir, 'gradient-{0}_left_medial.png'.format(index)))
    plotting.plot_surf_stat_map(grad_dict['pial_left'], grad_lh, hemi='left', bg_map=grad_dict['sulc_left'], bg_on_data = True, colorbar=False, view='lateral', cmap='jet', output_file=op.join(outdir, 'gradient-{0}_left_lateral.png'.format(index)))
    ...
    if np.max(grad_rh[grad_rh>0]) > np.abs(np.min(grad_rh[grad_rh<0])):
        ...
        grad_rh[grad_rh>0] = grad_rh[grad_rh>0]/np.max(grad_rh[grad_rh>0])
        grad_rh[grad_rh>0] = grad_rh[grad_rh>0]*np.abs(np.min(grad_rh[grad_rh<0]))
    plotting.plot_surf_stat_map(grad_dict['pial_right'], grad_rh, hemi='right', bg_map=grad_dict['sulc_right'], bg_on_data = True, colorbar=False, view='medial', cmap='jet', output_file=op.join(outdir, 'gradient-{0}_right_medial.png'.format(index)))
    plotting.plot_surf_stat_map(grad_dict['pial_right'], grad_rh, hemi='right', bg_map=grad_dict['sulc_right'], bg_on_data = True, colorbar=True, view='lateral', cmap='jet', output_file=op.join(outdir, 'gradient-{0}_right_lateral.png'.format(index)))
            ...
            if args.atlas is not None:
                ...
                if args.atlas != "fsaverage5" and args.atlas != 'hcp':
                    ...
                    if args.atlas == 'fsaverage5':
                        ...
                        sulc_left = nib.load('./templates/S1200.sulc_MSMAll.32k_fs_LR.dscalar.nii').get_fdata()[0][0:left_verts]
                        sulc_left = np.insert(sulc_left, np.subtract(medial_wall_inds_left, np.arange(len(medial_wall_inds_left))), 0)
                        sulc_right = nib.load('./templates/S1200.sulc_MSMAll.32k_fs_LR.dscalar.nii').get_fdata()[0][left_verts:]
                        sulc_right = np.insert(sulc_right, np.subtract(medial_wall_inds_right, np.arange(len(medial_wall_inds_right))), 0)
                ...
                if args.affinity == "cosine":
                    time_series = affinity(time_series, 10*args.sparsity)
