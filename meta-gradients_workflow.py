...
from nilearn import datasets, masking, surface
...
def main(
    ...
    subcortical_name = "subcortical-{0}".format(subcort)
    ...
    if dset:
        ...
        if term is not None:
            ...
            with open(op.join(workdir, "analysis-information.txt"), "a+") as fo:
                fo.write(
                    ...
                    "with the term {}\n".format(len(term_ids), len(dset.ids), term)
                )
        ...
        if neurosynth or (sleuth_file is not None) or (nimare_dataset is not None):
    ...
    if atlas is not None:
        if atlas == "harvard-oxford":
            ...
            # atlas_labels = atlas.labels[3:]  # assigned to but never used
            atlas_shape = nib.load(atlas.maps).shape
            atlas_affine = nib.load(atlas.maps).affine
            atlas_data = nib.load(atlas.maps).get_fdata()
        elif atlas == "msdl":
        ...
        if atlas != "fsaverage5" and atlas != "hcp":
            ...
            if atlas == "fsaverage5":
                ...
                sulc_left = (
                    nib.load("./templates/S1200.sulc_MSMAll.32k_fs_LR.dscalar.nii").get_fdata()[0][
                        0:left_verts
                    ]
                    * -1
                )
            ...
            with open(op.join(workdir, "analysis-information.txt"), "a+") as fo:
                fo.write(
                    (
                        f"{lh_vertices_total} vertices in left hemisphere after conversion to "
                        f"{atlas} surface space\n"
                    )
                )
                fo.write(
                    (
                        f"{rh_vertices_total} vertices in right hemisphere after conversion to "
                        f"{atlas} surface space\n"
                    )
                )
            ...
            ale_dict = {
                "grads_lh": np.expand_dims(surf_ale_array_lh, axis=1),
            ...
            im_list = [
                op.join(workdir, "ale-0_left_lateral.png"),
            ...
            if subcort:
                ...
                with open(op.join(workdir, "analysis-information.txt"), "a+") as fo:
                    fo.write(
                        f"{lh_vertices_wo_medial_wall} vertices in left hemisphere after removing "
                        f"{len(medial_wall_inds_left)} left medial wall vertices\n"
                    )
                    fo.write(
                        f"{rh_vertices_wo_medial_wall} vertices in right hemisphere after "
                        f"removing {len(medial_wall_inds_right)} right medial wall vertices\n"
                    )
                ...
                with open(op.join(workdir, "analysis-information.txt"), "a+") as fo:
                    fo.write(
                        "Adding time-series for {} sub-cortical voxels\n".format(num_subcort_vox)
                    )
        ...
        with open(op.join(workdir, "analysis-information.txt"), "a+") as fo:
            fo.write(
                "Matrix contains {0} voxels/vertices across {1} MA images\n".format(
                    np.shape(time_series)[1], np.shape(time_series)[0]
                )
            )
        time_series = time_series.astype("float32")

        inds_discard = np.append(
            np.where(np.isnan(np.mean(time_series, axis=0)))[0],
            np.where(~np.any(time_series, axis=0))[0],
        )
        if inds_discard.any():
            time_series = np.delete(time_series, inds_discard, axis=1)
            with open(op.join(workdir, "analysis-information.txt"), "a+") as fo:
                fo.write(
                    "removing {} vertices and voxels without MA values\n".format(len(inds_discard))
                )
                fo.write(
                    "Matrix contains {0} voxels/vertices across {1} MA images\n".format(
                        np.shape(time_series)[1], np.shape(time_series)[0]
    ...
    n_components = (
        np.where(np.cumsum(statistics["lambdas"] / np.sum(statistics["lambdas"])) > 0.5)[0][0] + 1
    )
    ...
    for i in range(n_components):
        if atlas is not None:
            if atlas == "fsaverage5" or atlas == "hcp":
                ...
                im_list = [
                    op.join(workdir, "gradient-{}_left_lateral.png".format(i)),
    ...
    output_dir = op.join(
        ...
        (
            ...
            f"{affinity_name}_{approach_name}_{subcortical_name}"
        ),
    )
